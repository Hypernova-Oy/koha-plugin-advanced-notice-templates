...
use Koha::Plugin::Fi::Hypernova::AdvancedNoticeTemplates::Exceptions::Commit;
use Koha::Plugin::Fi::Hypernova::AdvancedNoticeTemplates::Exceptions::SRUServer;
...
sub get_modifiable_templates {
    ...
    my $letter_codes = $params->{'letter_codes'} ? $params->{'letter_codes'} : undef;
    my $module       = $params->{'module'} // { -like => '%' };

    my $plugin_templates = $self->load_default_plugin_templates(
        {
    ...
    foreach my $lang ( keys %$plugin_templates ) {
        my $is_default_language = $self->retrieve_data('default_language') eq $lang;
        foreach my $letter_code ( $letter_codes ? @$letter_codes : keys %{ $plugin_templates->{$lang} } ) {
            my @modifiable_templates_this_code;
            my $template_name = $plugin_templates->{$lang}->{$letter_code}->{'name'};

            foreach my $mtt ( keys %{ $plugin_templates->{$lang}->{$letter_code}->{'template'} } ) {

                # avoid having to re-retrieve data on each iteration
                my $is_html;
                if ( exists $retrieved_data->{"use_html_$mtt"} ) {
                    $is_html = $retrieved_data->{"use_html_$mtt"} ? 1 : 0;
                } else {
                    $is_html = $retrieved_data->{"use_html_$mtt"} = $self->retrieve_data("use_html_$mtt") ? 1 : 0;
                }
                ...
                # avoid having to re-retrieve data on each iteration
                my $is_anonymous;
                if ( exists $retrieved_data->{"use_anonymous_$mtt"} ) {
                    $is_anonymous = $retrieved_data->{"use_anonymous_$mtt"} ? 1 : 0;
                } else {
                    $is_anonymous = $retrieved_data->{"use_anonymous_$mtt"} =
                        $self->retrieve_data("use_anonymous_$mtt") ? 1 : 0;
                }
                ...
                foreach my $current_language ( @{ $is_default_language ? [ 'default', $lang ] : [$lang] } ) {
                    ...
                    if (
                        ...
                        || ( $current_template && (   $current_template->content ne $plugin_template->{'content'}
                        ...
                        )
                    {
                        push(
                            @{ $modifiable_templates->{$letter_code} },
                            {
                                ...
                                name                   => $plugin_templates->{$lang}->{$letter_code}->{'name'},
                                is_html                => $is_html,
    ...
    foreach my $lang (@$languages) {
        ...
        if ( $params->{'letter_codes'} ) {
            ...
            foreach my $template_filepath ( glob( $path . '*.yml' ) ) {
                ...
                my $yaml        = YAML::XS::LoadFile($template_filepath);
                $templates->{$lang_dir}->{$letter_code}->{'name'}     = $yaml->{'name'};
